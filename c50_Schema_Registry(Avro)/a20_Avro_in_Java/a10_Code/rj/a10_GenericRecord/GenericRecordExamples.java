...
public class GenericRecordExamples {
    ...
    public static void main(String[] args) {
        ...
        // We can either parse file or string
        ...
        // Create first record (First customer)
        ...
        // Note: automate_email is not set, but still will be set because of default
        GenericData.Record customerWithDefault = customerBuilderWithDefault.build();
        System.out.println(customerWithDefault);

        // Third record (Missing field, Field with wrong type)
        GenericRecordBuilder customerWrong = new GenericRecordBuilder(schema);
        // Note: ** first_name is not set, but this give *runtime exception, as we do not have
        //          default value for the this field
        customerWrong.set("last_name", "Doe");
        customerWrong.set("age", 26);
        // Does this give run time error: ???
        customerWrong.set("height", "blahblah"); //**Bad (This should be float)
        ...
        try (DataFileReader<GenericRecord> dataFileReader = new DataFileReader<>(file, datumReader)){
            ...
            // Returns null** : When there is a non existent field
            // Again no static *type safety as we might get null at run time
